...
public class TcpTransport(ILoggerFactory loggerFactory, TransportReturnHandler returnEvent) : ITransport
{
    private readonly ILogger _logger = loggerFactory.CreateLogger<TcpTransport>();
    private readonly TransportReturnHandler _returnEvent = returnEvent;
    private readonly PacketBuffer _receiveBuffer = new PacketBuffer();

    private Socket _socket = null!;
    private SocketAsyncEventArgs _receiveArgs = null!;
    private ISessionNotify _notify = null!;

    private bool _isClosed;
    private readonly object _lockRelease = new();

    public void BindNotify(ISessionNotify notify) => _notify = notify;
    ...
    {
        _receiveBuffer.TryAppend(e.MemoryBuffer, e.BytesTransferred);
        while (_receiveBuffer.TryGetValidatePacket(out var packetType, out var payload, out var rentedBuffer))
        {
            await _notify.OnNotifyReceived(packetType, payload, rentedBuffer);
        }
    }
