...
public interface INetworkManager
{
    ...
    void Stop();
    Task OnSendAsync(string sessionId, Memory<byte> data, byte[]? rentedBuffer, Socket socket);
}
...
public class NetworkManager : INetworkManager
{
    ...
    public void Run(TransportType transportType, int port)
    {
        ...
        switch (transportType)
        {
            case TransportType.Tcp:
                ...
                _tcpAcceptor = new TcpAcceptor(_loggerFactory, OnClientConnected);

                _ = _tcpAcceptor.Run(port);
                break;
            case TransportType.Udp:

                _udpSender = new UdpSender(_loggerFactory, _poolEvent, new TransportUdpSendHandler { OnUdpPresetSend = OnUdpPresetSend });
                _udpReceiver = new UdpReceiver(_loggerFactory, new TransportUdpReceiveHandler { OnUdpGreetingReceived = OnUdpGreetingReceived, OnUdpDatagramReceived = OnUdpDatagramReceived });
    ...
    public async Task OnSendAsync(string sessionId, Memory<byte> data, byte[]? rentedBuffer, Socket socket)
    {
