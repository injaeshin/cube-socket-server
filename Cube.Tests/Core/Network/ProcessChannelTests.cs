...
public class ProcessChannelTests
{
    ...
    public async Task ProcessChannel_EnqueueAndProcess_ShouldWorkCorrectly()
    {
        ...
        using var channel = new ProcessChannel<ReceivedContext>(_loggerFactory, OnProcess);

        // Act
        var data = new byte[] { 1, 2, 3, 4 };
        await channel.EnqueueAsync(new ReceivedContext("t_session", PacketType.Ping, data, null));
