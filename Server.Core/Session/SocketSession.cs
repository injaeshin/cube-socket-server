...
{
    public ISocketSession Session { get; } = session;
}

public class SessionDataEventArgs(ISocketSession session, PacketType type, ReadOnlyMemory<byte> data) : SessionEventArgs(session)
{
    public PacketType PacketType { get; } = type;
    public ReadOnlyMemory<byte> Data { get; } = data;
}

public class SocketSession(SocketSessionOptions options, ILogger<SocketSession> logger) : ISocketSession, IDisposable
{
    ...
    public void CreateSessionId()
    {
        ...
        // // 로깅에 편리한 짧은 ID 생성 (마지막 8자리만)
        // string shortId = SessionId.Length > 8 ? SessionId.Substring(SessionId.Length - 8) : SessionId;
        // _logger.LogDebug("Session created: {SessionId} (Short: {ShortId})", SessionId, shortId);
    }
    ...
    protected virtual void OnConnect(ISocketSession session)
    {
        _logger.LogDebug("[Session Connected] {SessionId} {RemoteEndPoint}", SessionId, _socket.RemoteEndPoint);
    }

    protected virtual void OnDisconnect(ISocketSession session, DisconnectReason reason)
    {
